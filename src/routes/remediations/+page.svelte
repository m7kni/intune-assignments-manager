<script lang="ts">
	...
	import RemediationRow from '$lib/components/ui/RemediationRow.svelte';
	import { Search, Stethoscope, ArrowDownAZ, ArrowUpZA, Clock } from 'lucide-svelte';
	...
	let scripts = $state<DeviceHealthScript[]>([]);
	let search = $state('');
	...
	async function fetchScripts(): Promise<void> {
		...
		try {
			...
			scripts = await listRemediationScripts(client);
		} catch (err) {
	...
	>
		<div class="animate-fade-in-up">
			<PageHeader
				...
				icon={Stethoscope}
				description="Browse and manage your Intune proactive remediation scripts"
			/>

			<!-- Filter bar -->
			<div class="mb-4 flex flex-wrap items-center gap-3">
				<div class="min-w-0 flex-1">
					<SearchInput placeholder="Filter scripts by name or publisher..." bind:value={search} />
				</div>
				<DropdownMenu
					items={sortOptions}
					selected={sortBy}
